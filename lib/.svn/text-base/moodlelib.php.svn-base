 ...
 *
 */
function generate_email_signoff($u = null) {
    global $CFG;
    ...
    if(!empty($u)){
        if(!empty($u->email)){
            $result = strpos($u->email, "fiu");
            if($result > 0){
                $signoff = generate_kaseya_email_signoff();
            }
        }
    }
    return $signoff;
 ...
 *
 */
function generate_kaseya_email_signoff() {
    global $CFG;

    $signoff = "\n";
    $signoff .= "Kaseya University Team\n";
    $signoff .= "training@kaseya.com\n";
 ...
 *          was blocked by user and "false" if there was another sort of error.
 */
 // -------------------------------------------------------- Edited: JAM - 11.19.2010
 function send_confirmation_email($user) {	// sent to both the user and admin
	/*
		This function has been altered form its orginal state to disable user from being able to
		confirm their own user accounts.
		Special Defaults were created in the /lang/en_utf8/moodle.php (493 -522) for ensure the correct email text (EN).
	*/
    global $CFG;
    ...
    $emailsubject = new object();
    $emailsubject->sitename = format_string($site->fullname);
    $emailsubject->fullname = fullname($user);
	...
	*/
	...
	// Altered: JAM - for custom email without confirmation link
	$subject = get_string('emailadminconfirmationsubject', '', $emailsubject);
	
	// Altered: SMS - begin
	// For sending he confirmation link to the student, use the first alternative.
	// Alternative 1
	$data->link = $CFG->wwwroot .'/login/confirm.php?data='. $user->secret .'/'. urlencode($user->username);
    $message     = get_string('emailadminconfirmationwithlink', '', $data);
	$messagehtml = text_to_html(get_string('emailadminconfirmationwithlink', '', $data), false, false, true);
	// If you do not want to send the confirmation link to the student user alternative 2.
	// Alternative 2
    // $data->link = $CFG->wwwroot .'/login/';
    // $message     = get_string('emailadminconfirmation', '', $data);
    // $messagehtml = text_to_html(get_string('emailadminconfirmation', '', $data), false, false, true);
	// Altered: SMS - end
	
	// Add: JAM for the admin email 
	$data->link = $CFG->wwwroot .'/login/confirm.php?data='. $user->secret .'/'. urlencode($user->username);
    $amessage     = get_string('sendemailadminconfirmation', '', $data);
    $amessagehtml = text_to_html(get_string('sendemailadminconfirmation', '', $data), false, false, true);
    ...
    $otheruser->email = 'jamerrig@gmail.com';
	//*/
	$confirm = email_to_user($supportuser, $supportuser, $subject, $amessage, $amessagehtml);

    return email_to_user($user, $supportuser, $subject, $message, $messagehtml);

}
 // -------------------------------------------------------- Edited: JAM - 05.07.2012
 function send_enrollment_notification($course,$user) {	// sent to the admin
	/*
		This function has been added.
		Special Defaults were created in the /lang/en_utf8/moodle.php (538 -549) for ensure the correct email text (EN).
	*/
    global $CFG;
    ...
    // Altered: JAM - for custom email without confirmation link
    $subject = get_string('sendenrolmentnotifcationsubject', '', $coursedata);
    
    // Add: JAM for the admin email 
    $amessage     = get_string('sendenrolmentnotifcation', '', $data);
    $amessagehtml = text_to_html(get_string('sendenrolmentnotifcation', '', $data), false, false, true);

    $user->mailformat = 1;  // Always send HTML version as well
    
    //if($course->adminemailnotificaton)
       email_to_user($supportuser, $supportuser, $subject, $amessage, $amessagehtml);

}

function send_unenrollment_notification($course,$user) {	// sent to the admin
    global $CFG;
    ...
    // Altered: JAM - for custom email without confirmation link
    $subject = get_string('sendunenrolmentnotifcationsubject', '', $coursedata);

    // Add: JAM for the admin email 
    $amessage     = get_string('sendunenrolmentnotifcation', '', $data);
    $amessagehtml = text_to_html(get_string('sendunenrolmentnotifcation', '', $data), false, false, true);
...
// sms: 7/6/2014 Added to support embedded version
function send_user_deleted_notification($user) {	// sent to the admin
	echo "<br> inside send_user_deleted_notification";
	echo "<br> \$user->username is $user->username";
	
    global $CFG;

    $supportuser = generate_email_supportuser();
	echo "<br> \$supportuser->email is $supportuser->email";
    ...
    $subject = get_string('senddeletednotifcationsubject', '', $data);

    $amessage     = get_string('senddeletednotifcation', '', $data);
    $amessagehtml = text_to_html(get_string('senddeletednotifcation', '', $data), false, false, true);

    $user->mailformat = 1;  // Always send HTML version as well
    
    echo "<br> inside send_user_deleted_notification before email_to_user";
	echo "<br> \$subject is $subject";
	echo "<br> \$amessage is $amessage";
	echo "<br> \$amessagehtml is $amessagehtml";
	
	//if($course->adminemailnotificaton)
       email_to_user($supportuser, $supportuser, $subject, $amessage, $amessagehtml);
    echo "<br> inside send_user_deleted_notification after email_to_user";

}
// -------------------------------------------------------- Added: JAM - 11.19.2010
function admin_confirmation_email($user) {	// sent to the user
	...
	$subject = get_string('confirmationuseremailsubject', '', format_string($site->fullname));
	
	$data->link = $CFG->wwwroot .'/login/';
    $message     = get_string('confirmationuseremail', '', $data);
    $messagehtml = text_to_html(get_string('confirmationuseremail', '', $data), false, false, true);
...
function admin_signuperror_email($user) { // Sent to the Admin
	...
	$subject = get_string('signuperroradminemailsubject', '', format_string($site->fullname));
	
	$data->link = $CFG->wwwroot .'/login/';
    $message     = get_string('signuperroradminemail', '', $data);
    $messagehtml = text_to_html(get_string('signuperroradminemail', '', $data), false, false, true);
...
function enrolment_request_email($user, $course) { // Sent to the Admin
...
function admin_moodlefailed_email($user, $type, $course) { // Sent to the Admin
	
	global $CFG;

    $type = 'Moodle error';
	$subjecttext = 'moodleerroremailsubject';
	admin_customizederror_email($subjecttext, $type, $user, $action, $course);
}

function admin_webservicefailed_email($user, $action, $course) { // Sent to the Admin
	
	global $CFG;
	
	$type = 'Web Service error';
	$subjecttext = 'webserviceerroremailsubject';
	admin_customizederror_email($subjecttext, $type, $user, $action, $course);

}

function admin_customizederror_email($subjecttext, $type, $user, $action, $course) { // Sent to the Admin
	...
	if($action == 'addUser' || $action == 'deleteUser' || $action == 'modifyUser' || $action == 'modifyUserPassword'){
		$sendemail = true;
		$data = new object();
		$data->label = 'User';
	...
	if($action == 'unenrollUser' || $action == 'enrollUser'){
		$sendemail = true;
		$data = new object();
		$data->userlabel = 'Course:';
		$data->requester = $course->fullname;
		$data->label = 'User';
		$data->name = fullname($user);
		$data->id = $user->id;
		$data->email = $user->email;
	...
	if($action == 'deleteCourse' || $action == 'modifyCourse' || $action == 'addCourse'){
		$sendemail = true;
		$data = new object();
		$data->userlabel = 'Requester:';
		$data->requester = fullname($user);
		$data->label = 'Course';
	...
	if($sendemail){
		...
		$subject = get_string($subjecttext, '', format_string($site->fullname));
		
		//$data->link = $CFG->wwwroot .'/login/';
		$message     = get_string('moodle_webserviceerroremail', '', $data);
		$messagehtml = text_to_html(get_string('moodle_webserviceerroremail', '', $data), false, false, true);
...
// -------------------------------------------------------- End of Addition
// -------------------------------------------------------- Added: JAM 11/23/2010

function appointment_confirmation_email($user, $course, $resource, $appointments) {
	...
	$subject = get_string('appointmentconfirmationemailsubject', '', format_string($site->fullname));
	...
	if(is_array($appointments)){
		for ($j=0; $j < count($appointments); $j++){
			...
			$appointmentinfo .= "\t".strftime($format,strtotime($appointment->start))." to ".strftime($format,strtotime($appointment->end))." - ";
			$appointmentinfo .= $appointment->course.": ".$appointment->resourceType."\n";
			if($j==0){
				$data->starttime = strftime("%H:%I %p",strtotime($appointment->start));
				$data->endtime = strftime("%H:%I %p",strtotime($appointment->end));
			}
		}
	}else{
		$appointmentinfo .= "\t".strftime($format,strtotime($appointments->start))." to ".strftime($format,strtotime($appointments->end))." - ";
		$appointmentinfo .= $appointments->course.": ".$appointments->resourceType."\n";
		$data->starttime = strftime("%H:%I %p",strtotime($appointments->start));
		$data->endtime = strftime("%H:%I %p",strtotime($appointments->end));
	}

	$data->portal = $CFG->wwwroot .'/login/';
	//$data->link = "LINK";
	$data->fromemail = $supportuser->email;
	$data->resourcetype = $resource;
	$data->course = $course;
	$data->confirmappointments = $appointmentinfo;
	
    $message     = get_string('appointmentconfirmationemail', '', $data);
    $messagehtml = text_to_html(get_string('appointmentconfirmationemail', '', $data), false, false, true);
...
function appointment_cancellation_email($user, $appointments) {
	...
	$subject = get_string('appointmentcancellationemailsubject', '', format_string($site->fullname));
	
	$appointmentinfo = "";
	
	$format = "%a %b %d, %Y, %H:%I %p";
	
	if(is_array($appointments)){
		for ($j=0; $j < count($appointments); $j++){
			$appointment = $appointments[$j];
			$appointmentinfo .= "\t".$appointment->start." to ".$appointment->end." - ";
			$appointmentinfo .= $appointment->course.": ".$appointment->resourceType."\n";
		}
		$data->course = 'multiple course';
	}else{
		$appointmentinfo .= "\t".$appointments->start." to ".$appointments->end." - ";
		$appointmentinfo .= $appointments->course.": ".$appointments->resourceType."\n";
		$data->course = $appointments->course;
	}
	...
	$data->fromemail = $supportuser->email;
	$data->resourcetype = $resource;
	$data->cancelledappointments = $appointmentinfo;
	
    $message     = get_string('appointmentcancellationemail', '', $data);
    $messagehtml = text_to_html(get_string('appointmentcancellationemail', '', $data), false, false, true);
...
function appointment_modification_email($user, $oldappointments, $newappointments) {
	...
	$subject = get_string('appointmentmodificationemailsubject', '', format_string($site->fullname));
	...
	if(is_array($oldappointments)){
		for ($j=0; $j < count($oldappointments); $j++){
			$appointment = $oldappointments[$j];
			$oldappointmentinfo .= "\t".$appointment->start." to ".$appointment->end." - ";
			$oldappointmentinfo .= $appointment->course.": ".$appointment->resourceType."\n";
		}
		$data->course = 'multiple course';
	}else{
		$oldappointmentinfo .= "\t".$oldappointments->start." to ".$oldappointments->end." - ";
		$oldappointmentinfo .= $oldappointments->course.": ".$oldappointments->resourceType."\n";
		$data->course = $oldappointments->course;
	}
	
	if(is_array($newappointments)){
		for ($j=0; $j < count($newappointments); $j++){
			$appointment = $newappointments[$j];
			$newappointmentinfo .= "\t".$appointment->start." to ".$appointment->end." - ";
			$newappointmentinfo .= $appointment->course.": ".$appointment->type."\n";		}
	}else{
		$newappointmentinfo .= "\t".$newappointments->start." to ".$newappointments->end." - ";
		$newappointmentinfo .= $newappointments->course.": ".$newappointments->type."\n";
	}
	...
	$data->fromemail = $supportuser->email;
	$data->resourcetype = $resource;
	$data->originalappointmentinfo = $oldappointmentinfo;
	$data->newappointmentinfo = $newappointmentinfo;
	
    $message     = get_string('appointmentmodificationemail', '', $data);
    $messagehtml = text_to_html(get_string('appointmentmodificationemail', '', $data), false, false, true);
