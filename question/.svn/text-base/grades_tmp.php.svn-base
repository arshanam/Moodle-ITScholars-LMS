...
<script type="text/javascript" src="../ixedit-pack/jquery/jquery-plus-jquery-ui.js"></script>
...
<?php
    ...
    if (!empty($param->uid)) {
        ...
        if($cnt){
            ...
            $mainlist = $quizzes;
            printJQueryScript($quizzes);
            
            if(!empty($quizzes)){

                $javascript = "";

                echo "<table id='mainlist' border='0' cellpadding='5' cellspacing='5'>";
                echo "<tr><td valign='top'>";
                echo "<ul type='none'>";
                foreach ($mainlist as $listitem) {

                echo "<li id='$listitem->id'><a id='$listitem->id'>$listitem->name</a></li>";
...
function printJQueryScript($quizzes){
    echo "<script type='text/javascript'>";
    echo "  if(window.ixedit){ixedit.deployed = true};";
    echo "  if(window.jQuery){jQuery(function(){";
    echo "      (function(){ var target = jQuery('li'); target.tabs({event:'click'}); var target = jQuery('table.grades'); target.hide(0); })();";
    foreach ($quizzes as $quiz) {
        //echo "	(function(){ jQuery('li#$quiz->id').bind('click', function(event, ui){var target = jQuery('table.grades'); target.hide(0); var target = jQuery('table#tablesorter_$quiz->id'); target.show(0)});})();";

        echo "	(function(){ jQuery('li#$quiz->id').bind('click', function(event, ui){var target = jQuery('table.grades'); target.hide(0); var target = jQuery('div.inf'); target.hide(0); var target = jQuery('div#inf_tablesorter_$quiz->id'); target.show(0); var target = jQuery('table#tablesorter_$quiz->id'); target.show(0)});})();";
