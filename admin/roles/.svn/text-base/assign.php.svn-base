        ...
        if ($add and !empty($frm->addselect) and confirm_sesskey()) {
            ...
            foreach ($frm->addselect as $adduser) {
                ...
                if ($allow) {
					...
					//modifyQSUser(get_record('user', 'id', adduser));	// Added: 01.03.2011 - quotasystem.php
					$tmpuser = get_record('user','id',$adduser);
					...
					if (! role_assign($roleid, $adduser, 0, $context->id, $timestart, $timeend, $hidden)) {
					    ...
					    send_enrollment_notification($course,$tmpuser);	// Added: 05/08/2012
					}
					
					// Added: to call enrollment functions for the course
					if(!enrollQSUser($tmpuser, $course)){		//Added: 12.30.2010
						if (! role_unassign(0, $adduser, 0, $context->id)) {
							$errors[] = "Could not remove failed add user attempt, with id $adduser to this role! (QS WS)";
						}
						
					}else{
						if(!enrollUserInCourse($USER->username, $tmpuser->username, $course->fullname, true)){	
							if (! role_unassign(0, $adduser, 0, $context->id)) {
								$errors[] = "Could not remove failed add user attempt, with id $adduser to this role! (VL WS)";
							}
							unenrollQSUser($tmpuser, $course);
							
						}
					}
					
					// Added: to call calendar ws to edit user role.
					editUserProfileRole($USER->username, $adduser, $roleid, true);
            ...
            foreach ($frm->removeselect as $removeuser) {
				...
				$tmpuser = get_record('user','id',$removeuser);
				...
				if(unenrollQSUser($tmpuser, $course)){
					if(enrollUserInCourse($USER->username, $tmpuser->username, $course->fullname, false)){
						...
						if (! role_unassign($roleid, $removeuser, 0, $context->id)) {
							$errors[] = "Could not remove user with id $removeuser from this role! [3]";
						} else if ($inmeta) {
						...
						$errors[] = "Could not remove user with id $removeuser from this role! [2]";
					}
				}else{
					$errors[] = "Could not remove user with id $removeuser from this role! [1]";
				}
			
				// Added: to call calendar ws to edit user role.
				editUserProfileRole($USER->username, $removeuser, $roleid, false);
