<?php // $Id: confirm.php,v 1.29.2.1 2008/04/02 06:10:02 dongsheng Exp $
    ...
    if (!empty($data) || (!empty($p) && !empty($s))) {
        ...
        if ($confirmed == AUTH_CONFIRM_ALREADY) {
			...
			/*
			// Added: to call calendar ws to create user. - NEW ************************
			if(!createUserProfile($USER->username, $USER)){
				error("Account confirmation could not be completed (ws).");
			}
			*/
            
			//addQSUser($USER);
			admin_confirmation_email($USER);		// Added: JAM 11.19.2010 - sends email to user inform them of confirmation
			
			//set_moodle_cookie($USER->username);		// Commented: JAM 11.19.2010 - Only Admin is confirming (Don't Login)
