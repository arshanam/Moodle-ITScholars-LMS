<?php // $Id: myindex.php,v 1.129.2.5 2009/03/23 09:46:18 mudrd8mz Exp $


    require_once("../config.php");

    // check if major upgrade needed - also present in /,yindex.php
...
/// Define variables used in page
    ...
    if (empty($CFG->langmenu)) {
        ...
        $langmenu = popup_form ("$CFG->httpswwwroot/login/myindex.php?lang=", $langs, "chooselang", $currlang, "", "", "", true, 'self', $langlabel);
    }
...
/// Check if the user has actually submitted login data to us
    ...
    if (empty($CFG->usesid) and $testcookies and (get_moodle_cookie() == '')) {    // Login without cookie when test requested
        echo "$CFG->usesid: ".$CFG->usesid;
        echo "$testcookies: ".$testcookies;
        echo "get_moodle_cookie(): ".get_moodle_cookie();
        ...
        if ($user) {
            ...
            if (empty($errormsg)) {
				// Added: JAM 04.01.2012 - for the first time users imported
				//if(!on_login_change($frm->username)){
        ...
        /// Go to my-moodle page instead of homepage if mymoodleredirect enabled
            if (!has_capability('moodle/site:config',get_context_instance(CONTEXT_SYSTEM)) and !empty($CFG->mymoodleredirect) and !isguest()) {
                if ($urltogo == $CFG->wwwroot or $urltogo == $CFG->wwwroot.'/' or $urltogo == $CFG->wwwroot.'/myindex.php') {
        ...
        /// Currently supported only for ldap-authentication module
            ...
            // JAM Added: 03.08.2012 - Redirects for the admin
            if ($user->username == 'admin') {
                $urltogo = $CFG->wwwroot.'/schedule.php';
            }
...
/// First, let's remember where the user was trying to get to before they got here
    ...
    if (empty($SESSION->wantsurl)) {
        $SESSION->wantsurl = (array_key_exists('HTTP_REFERER',$_SERVER) &&
                              ...
                              $_SERVER["HTTP_REFERER"] != $CFG->httpswwwroot.'/login/myindex.php')
...
/// Generate the login page with forms
    ...
    if (empty($frm->username) && $authsequence[0] != 'shibboleth') {  // See bug 5184
        if (!empty($_GET["username"])) {
            ...
            // ADDED: JAM -- 06.20.2014
            if (!empty($_GET["password"])) {
                $frm->password = $_GET["password"];
            }else{
                $frm->password = "";
            }
        } else {
    ...
    include("myindex_form.html");
    
    // ADDED: JAM -- 06.20.2014
    if (!empty($_GET["auto"]))
        echo "<script type='text/javascript'>$('form#login').submit();</script>";

    print_footer();

/*************************************** Functions */
/*
function on_login_change($username){

	/// JAM: Added for imported users on first-time login
	$user = get_record('user','username',$username);
	$on_import = get_record('user_info_data','userid',$user->id,'fieldid',17);
	
	if($on_import->data == "reset")
		return false;
	else
		return true;
