    ...
    if ((stripslashes($form->password) == $course->password) or ($groupid !== false) ) {
        ...
        if (isguestuser()) { // only real user guest, do not use this for users with guest role
			...
			if(enrollUserInCourse($USER->username, $USER->username, $course->fullname, true)){				
				...
				if (enrol_into_course($course, $USER, 'manual')) {
					// force a refresh of mycourses
					send_enrollment_notification($course,$USER);	// Added: 05/08/2012
					unset($USER->mycourses);
					if ($groupid !== false) {
					...
					if(!enrollQSUser($USER, $course)){		//Added: 01.06.2011
						admin_webservicefailed_email($USER,'enrollUser',$course);
					}
				}else{
...
function print_enrolmentkeyfrom($course) {
	...
	echo "<br/><a href='".$CFG->wwwroot."/course/enrol.php?id=".$course->id."&ekey=".$course->id."'>Request Enrolment Key</a>";
}
