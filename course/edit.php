...
/// process data if submitted
        ...
        if (empty($course)) {
			...
			if(!addQSCourse($course)){		// Added: JAM 12.30.2010 - sends ws call to create a course in quota system
				// id is required to create course in WS
				admin_webservicefailed_email($USER,'addCourse',$course);
			}		
			if(!addVLCourse($course)){		// Added: JAM 02.23.2012 - sends ws call to create a course in virtualLabs
				deleteQSCourse($course);
				admin_webservicefailed_email($USER,'addCourse',$course);
			}
