<?php // $Id: enrol.php,v 1.50.2.1 2008/03/03 05:27:36 moodler Exp $
    ...
    $enrolmentkey = optional_param('ekey', null, PARAM_INT);
...
/// Check the submitted enrolment information if there is any (eg could be enrolment key)
	...
	/// Added: Check value submitted and call calendar ws to enrol user
	////moved to another enrol section, to fix bug (/enrol/manual/enrol.php	94)
	...
	if(!empty($enrolmentkey) && $enrolmentkey > 0){
		enrolment_request_email($USER, $course);
		$message = "Your request was submitted to the administrator. <a href='".$CFG->wwwroot."'>Return to Main.</a>";
		$enrol->confirmed_request($course, $message);
