...
//--------------------------------------------------------------------------------
        // Added: JAM 04/27/2012
        $choices = array();
        $choices['0'] = get_string('no');
        $choices['1'] = get_string('yes');
        $mform->addElement('select', 'adminemailnotificaton', 'Notify Admin by Email', $choices);
        //$mform->setHelpButton('adminemailnotificaton', array('adminemailnotificaton', 'Notifies the admin by email when a user has enrolled in a course.'), true);
        $mform->setDefault('adminemailnotificaton', 1);
        
        // Added: JAM 02/13/2012
        $formcoursepromocodes = array();
		$sql_coursepromocodes = "SELECT code FROM mdl_course_promocodes";
	    $coursepromocodes = get_records_sql($sql_coursepromocodes);
		//$coursepromocodes = get_records('course_promocodes');
        foreach ($coursepromocodes as $coursepromocode) {
            $formcoursepromocodes["$coursepromocode->code"] = $coursepromocode->code;
        }
        $mform->addElement('select', 'promocode', get_string('promocode'), $formcoursepromocodes);
        //$mform->setHelpButton('format', array('courseformats', get_string('courseformats')), true);
        $mform->setDefault('promocode', '');
		// -- End
//--------------------------------------------------------------------------------		
