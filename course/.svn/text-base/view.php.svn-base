...
//  Display the course home page.
	...
	// sms: 7/8/2014 Added to support embedded version
	if (!empty($id)) {
		if (($id == 123) && ($USER->username != "admin") && ($USER->username != "123test")) { // Kaseya 7.0 Fundamentals Workshop
			redirect("http://university.kaseya.com");
		}
	}
	...
	if(isEnrolledInCourse($USER->username, $course->id)){
		if(!availableQuota($USER->username, $course->fullname)){
			error('You do not have enough quota available to view this course.<br/><br/> <a href="'.$CFG->wwwroot.'/mod/shoppingcart/view.php?id=156">Go to the Quota Store</a>');
		}
	}
	
	//http://ita-portal.cis.fiu.edu/moodle/mod/shoppingcart/view.php?id=156
