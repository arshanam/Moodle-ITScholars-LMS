...
///////////////////////////////////////////////////////////////////////////
   ...
   //Build Home Page Navigation - depending on login status
    if (isloggedin() and !isguest() and isset($CFG->frontpageloggedin)) {
       // Logged-In
       $navlinks = array(array('name' => "Scheduled Activities", 'link' => null, 'type' => 'misc'));
...
$(document).ready(function() {
	...
	var editservicecall = "editservicecall.php";
	var resetauths = "resetauths.php";
	var syncQS = "syncQS.php";
	
	
	$("#editservicecall").click(function(){
	
		var message = "Are you sure you want to request all user profiles to be updated?"
		noticeDialog("Confirmation", message, "alert", sendRequest, editservicecall);
		//$("#infomessage").show();
		//$("#infomessage").html("Working ...");
		//sendRequest(editservicecall);
		
	});
	$("#resetauths").click(function(){
	
		var message = "Are you sure you want to reset ALL user passwords?"
		noticeDialog("Confirmation", message, "alert", sendRequest, resetauths);
	
		//$("#infomessage").show();
		//$("#infomessage").html("Working ...");
		//sendRequest(resetauths);
	});
	$("#sync-qs").click(function(){
	
		var message = "Are you sure you want to sync moodle with the quota system database?"
		noticeDialog("Confirmation", message, "alert", sendRequest, syncQS);
	});
...
function sendRequest(requestedPage) {
	...
	$.ajax({
		...
		error: function(XMLHttpRequest, textStatus, errorThrown){
			...
			$("#infomessage").html("Your request could not be completed.");
	...
	setTimeout(function(){
		$("#infomessage").fadeOut("slow", function () {
			$("#infomessage").html("");
...
function noticeDialog(header, message, icon, completeCall, param){
...
// Start - Middle Container Content
    ...
    $setcerttest = "$CFG->wwwroot/setcerttest.php";
	$editCoursePromoCode = "$CFG->wwwroot/edit_promocodes.php";
    
    // Temp
    $clearexams = "$CFG->wwwroot/clearexm.php?";
    $listManual = "$CFG->wwwroot/ManualList.php";
	$editservicecall = "$CFG->wwwroot/editservicecall.php";
	$resetauths = "$CFG->wwwroot/resetauths.php";
    ...
    if (!empty($userRole)) {
        ...
        if ($userRole->roleid == 1) {   // Administrator Role
            ...
            echo "<li><a href='$listManual'>Moodle References</a></li>";
			echo "<li><a href='#' id ='editservicecall'>Register All User With Web Service</a></li>";
			echo "<li><a href='#' id='resetauths'>Reset Passwords</a></li>";
			echo "<li><a href='#' id='sync-qs'>Quota System User and Courses Sync</a></li>";
            echo "<li><a href='".$setcerttest."'>Manage Certification Tests</a></li>";
			echo "<li><a href='".$editCoursePromoCode."'>Edit Course Promo Code</a></li>";
            echo "</ol>";
			
			echo "<div id='infomessage' style='display: none'></div><br/>";
                            ...
                            if (!empty($records)) {
                                ...
                                foreach ($records as $record) {
                                    ...
                                    if($record->status == "Canceled"){
                                        ...
                                        $currentExam .= "<table border='0' cellpadding='5' cellspacing='5' width='780px'>";
...
function displayInfo($record, $diff, $return=false){
    ...
    $info .= "<b>";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDay($record->starttime))</script>";
    $info .= "</b> from ";
    //$info .= "<b>".date("l dS \of F Y",$record->starttime)."</b> from ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate($record->starttime))</script>";
    $info .= " to ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate($record->endtime))</script>";
...
function displayExamInfo($record, $diff, $return=false){
    ...
    $info .= "<b><script type='text/javascript'>document.write(formatMoodleDay($record->starttime))</script></b><br/>";
...
function showOrientation($record, $diff, $return=false){
    ...
    $info = "Orientation: available from ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate(".($record->starttime-($diff*60))."))</script>";
    $info .= " to ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate($record->endtime))</script>";
    $info .= "<br/>";
...
function showConcepts($record, $diff, $return=false){
    ...
    $info = "Part I (Concepts): starts at ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate($record->starttime))</script>";
    $info .= " and will be available until ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate(".($record->starttime+($diff*60))."))</script>";
    $info .= "<br/>";
...
function showImplementation($record, $diff, $return=false){
    ...
    $info = "Part II (Implementation): starts after you complete Exam Part I or if it is ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate(".($record->starttime+($diff*60))."))</script>";
    $info .= ", and is available until ";
    $info .= "<script type='text/javascript'>document.write(formatMoodleDate($record->endtime))</script>";
    $info .= "<br/><br/>";
...
function showGrades($record, $return=false){
    ...
    $report .= "<tr><td><b>Exam Part I:</b></td><td>$grade1 / $quiz->sumgrades</td><td><b>Exam Part II:</b></td><td>$grade2 / 100</td></tr>";
    $report .= "</table>";
...
function examStatus($record,$codes,$value="",$return=false){
  ...
  // REVISED:Mar 01 . 2010 - Also in view instructions
