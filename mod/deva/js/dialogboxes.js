...
function createInstantAppointment4CTDialogBox(username, course, type) {
	  
	$("#dialog").html("");
	var dialogContent = $("#dialog").load('forms/instant-appointment-4CT-form.html', function() {
	...
	$("#dialog").dialog({ 
		...
		width: 600,
		modal: true,
		title: "On-Demand Certification Test",
		close: function() {
			$(this).dialog("close");
		},
		buttons: { 
			"Confirm": function() { 
				...
				setTimeout(function(){	// Added: to allow the progress bar to appear.
					...
					var newevent = getCreateNewEventObjFromInstantApp4CTForm(username, course, type);
					if (scheduleAppointment(newevent, username)) {
						// sms: updated on 6/2/2011
						// var devaWasDisplayed = false;
						// for (var i=0; i<5; i++) {
							...
							// if (devaWasDisplayed) {
								// progressDialogBox(false);
								// alert("The virutal lab for your Certification Test is almost ready! " +
								// 		"If after trying to connect to any of your virtual machines, " +
								// 		"you receive a message  box with the title 'Terminal Server " +
								// 		"Connection Error' you should wait for 20 seconds and try again! " +
								// 		"Thank you for your patience! ");
						...
						// progressDialogBox(false);
						// if (!devaWasDisplayed) { 
						// 	alert("Your virutal lab for your Certification Test was successfully scheduled, " +
						// 			"but cannot yet be displayed. Please wait for a couple " +
						// 			"of minutes, refresh the page, and try again.");
						// }
					} else {
