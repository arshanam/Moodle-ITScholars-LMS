				...
				if (vms.success) {
					...
					*/
					...
					for (var i=0; i<vms.vmInfo.length; i++) {
						...
						$("#devaTabs").append('<a id="tab'+i+'" class="devaTabs ui-corner-all" '+
								'href="webRDP2.php?'+
									'tab=tab'+i+
									'&vmInsId='+vms.vmInfo[i].id+
									'&bottomFrameHeightPercentage='+bottomFrameHeightPercentage+
									'" target="mainscreen">'+
								'<span>'+vms.vmInfo[i].name+'</span></a>&nbsp;');
					}
...
{
	// alert("Hi!");
  document.getElementById('mainscreenid').style.height = parseInt(newHeight) + 10 + 'px';
}
