<?php  // $Id: view-embedded.php,v 1.124.2.18 2009/01/27 04:44:00 tjhunt Exp $

/**
 * This page prints a particular instance of quiz
 *
 * @author Masoud Sadjadi
 * @version $Id: view-embedded-with-encrypted-password.php,v 1.0.0.0 2014/07/11 
 ...
 */
    ...
    require_once($CFG->dirroot.'/mod/quiz/pagelib-embedded-with-encrypted-password.php');

    $id   = optional_param('id', 0, PARAM_INT); // Course Module ID, or
	$username = optional_param('username', 0, PARAM_TEXT); // username for embedded verions 
	// $username 	= $_GET["username"]; // username for embedded verions
    $q    = optional_param('q',  0, PARAM_INT);  // quiz ID
    $edit = optional_param('edit', -1, PARAM_BOOL);
/*
// check for admin role, teacher or creator
$userRole = get_record('role_assignments', 'userid', $USER->id);
if (empty($userRole) || ($userRole->roleid == 5)) {
    // Do Not Show This page - Allows for additional Attempts
    redirect("$CFG->wwwroot/schedule.php");
    //echo "HERE";
}
*/

	// echo "<br> id is $id and username is $username";
	if ($username) {
    ...
    add_to_log($course->id, "quiz", "view", "view-embedded-with-encrypted-password.php?id=$cm->id", $quiz->id, $cm->id);

    // echo "<br> quiz id is $quiz->id";
    ...
    if (!has_any_capability(array('mod/quiz:reviewmyattempts', 'mod/quiz:attempt', 'mod/quiz:preview'), $context)) {
        print_box('<p>' . get_string('youneedtoenrol', 'quiz') . '</p><p>' .
				...
				// print_continue($CFG->wwwroot . '/course/view-embedded-with-encrypted-password.php?id=' . $course->id, true) .
                print_continue('view-embedded-with-encrypted-password.php?id=' . $course->id, true) .
                // SMS End Change
				'</p>', 'generalbox', 'notice');
        finish_page($course);
    }
    ...
    if (!$quiz->questions) {
        ...
        if ($unfinished) {
            ...
            if (!empty($attempts)) {
                ...
                if ($tempunavailable) {
                    ...
                    print_continue($CFG->wwwroot . '/course/view-embedded.php?id=' . $course->id);
                    $buttontext = '';
                }
            }
        }
        ...
        if ($buttontext) {
            ...
            } else if ($quiz->timelimit || isCertificateTest($quiz->id,true)) {	//12.07.2011 - jam
				//29.06.2011 - jam
            ...
            $attempturl = "attempt-embedded-with-encrypted-password.php?id=$cm->id";
            ...
            if (!empty($quiz->popup)) {
                ...
                print_single_button("attempt-embedded-with-encrypted-password.php", array('id'=>$cm->id), $buttontext, 'get', '', false, '', false, $strconfirmstartattempt);
                // echo "\$cm->id is $cm->id";
				if ($cm->id == 10578) {
					print_single_button("../assignment/view-embedded.php", array('id'=>10579), 
						"Submit your answer file", 'get', '', false, 
						'Click to submit your answer file!', false, '');
				} else if ($cm->id == 10835) {
					print_single_button("../assignment/view-embedded.php", array('id'=>10838), 
						"Submit your answer file", 'get', '', false, 
						'Click to submit your answer file!', false, '');
				}
            }
...
function make_review_link($linktext, $quiz, $attempt, $context) {
    ...
    $url = "review-embedded.php?q=$quiz->id&amp;attempt=$attempt->id";
    if ($quiz->popup) {
