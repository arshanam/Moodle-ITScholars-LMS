<?php  // $Id: review-embedded.php,v 1.59.2.10 2009/01/16 04:47:35 tjhunt Exp $
/**
...
/// Set QCA Object
$qca = get_records('quiz_course_activation', 'username', $USER->username);
$starttime = $qca->starttime;
$endtime = $qca->endtime;

$now = time();
/// Show Review if Exam is over
if($now < $starttime){
    ...
    if (!has_capability('mod/quiz:viewreports', $context)) {
        ...
        if ($attempt->userid != $USER->id) {
            error("This is not your attempt!", 'view-embedded.php?q=' . $quiz->id);
        }
        // Check capabilities.
        ...
        if (!$options->responses) {
            ...
            if (empty($popup)) {
                redirect('view-embedded.php?q=' . $quiz->id, $message);
            } else {
...
/// Bits needed to print a good URL for this page.
    ...
    add_to_log($course->id, 'quiz', 'review', 'review-embedded.php?attempt=' . $attempt->id . $urloptions, $quiz->id, $cm->id);
...
/// Print the page header
    ...
    if (!empty($popup)) {
		...
		// SMS: 7/19/2014 Supporting the embedded version
...
/// Print heading.
    ...
    if (!$isteacher) {
        include('attempt_close_js-embedded.php');
    }
...
/// an array, then later we only output the table if there are any rows to show.
    ...
    if ($attempt->userid <> $USER->id) {
        ...
        $rows[] = '<tr><th scope="row" class="cell">' . $picture . '</th><td class="cell"><a href="' .
                $CFG->wwwroot . '/user/view-embedded.php?id=' . $student->id . '&amp;course=' . $course->id . '">' .
    ...
    /// List of all this user's attempts for people who can see reports.
        ...
        foreach ($attempts as $at) {
            if ($at->id == $attempt->id) {
                ...
                $attemptlist[] = '<a href="review-embedded.php?attempt=' . $at->id . $urloptions . ' ">' . $at->attempt . '</a>';
            }
        }
...
/// Print the navigation panel if required
    ...
    if ($numpages > 1 and !$showall) {
        print_paging_bar($numpages, $page, 1, 'review-embedded.php?attempt='.$attempt->id.'&amp;');
        echo '<div class="controls"><a href="review-embedded.php?attempt='.$attempt->id.'&amp;showall=true">';
        print_string('showall', 'quiz');
        echo '</a></div>';
    }

/// Print all the questions
    $quiz->thispageurl = $CFG->wwwroot . '/mod/quiz/review-embedded.php?attempt=' . $attempt->id . $urloptions;
    $quiz->cmid = $cm->id;
