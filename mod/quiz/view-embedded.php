 ...
 * @version $Id: view-embedded.php,v 1.0.0.0 2014/07/11 
 ...
 */
    ...
    require_once($CFG->dirroot.'/mod/quiz/pagelib-embedded.php');
...
*/
    ...
    add_to_log($course->id, "quiz", "view", "view-embedded.php?id=$cm->id", $quiz->id, $cm->id);
    ...
    if (!has_any_capability(array('mod/quiz:reviewmyattempts', 'mod/quiz:attempt', 'mod/quiz:preview'), $context)) {
        print_box('<p>' . get_string('youneedtoenrol', 'quiz') . '</p><p>' .
				...
				// print_continue($CFG->wwwroot . '/course/view-embedded.php?id=' . $course->id, true) .
                print_continue('view-embedded.php?id=' . $course->id, true) .
                // SMS End Change
				'</p>', 'generalbox', 'notice');
        finish_page($course);
    }
    ...
    if (!$quiz->questions) {
        ...
        if ($buttontext) {
            ...
            if (!empty($quiz->popup)) {
                ...
                print_single_button("attempt-embedded.php", array('id'=>$cm->id), $buttontext, 'get', '', false, '', false, $strconfirmstartattempt);
                // echo "\$cm->id is $cm->id";
				if ($cm->id == 10578) {
