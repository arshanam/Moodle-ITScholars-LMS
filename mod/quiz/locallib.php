...
function isCertificateTest($cmid,$isQuiz=false){
    $result = false;
	if(!$isQuiz){
		
		$cm = get_record('course_modules','id',$cmid);
		if(record_exists('certificationtest','quizid',$cm->instance)){
			$result = true;
		}
	}else{
		if(record_exists('certificationtest','quizid',$cmid)){
			$result = true;
		}
	}
    return $result;
...
function courseHasVirtualLabs($cmid){
    $result = false;
	...
	if(record_exists('course_modules','course',$cm->course,'module',43)){
		$result = true;
	}
...
function getFirstVirtualLabId($cmid){
	
	if ($cm = get_coursemodule_from_id('quiz', $cmid)) {
	
		$cms = get_record('course_modules','course',$cm->course,'module',43);
